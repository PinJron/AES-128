#include "aes.h"
#include "gmult.h"

//Number of columns
const int Nb = 4;

//Number of strings
const int Nk = 4;

//Number of rounds
const int Nr = 10;

static uint8_t s_box[256] = 
{
	 
};

static uint8_t inv_s_box[256] = 
{
	 
};


void sub_bytes(uint8_t *state) 
{
	for (uint8_t i = 0; i < 4; i++) 
	{
		for (uint8_t j = 0; j < Nb; j++) 
		{
			state[Nb*i + j] = s_box[state[Nb*i + j]];
		}
	}
}

void inv_sub_bytes(uint8_t *state) 
{
	for (uint8_t i = 0; i < 4; i++) 
	{
		for (uint8_t j = 0; j < Nb; j++) 
		{
			state[Nb*i + j] = inv_s_box[state[Nb*i + j]];
		}
	}
}

void shift_rows(uint8_t *state) 
{
	uint8_t s, tmp;
	for (uint8_t i = 1; i < 4; i++) 
	{
		s = 0;
		while (s < i) 
		{
			tmp = state[Nb*i + 0];
			for (uint8_t k = 1; k < Nb; k++) 
			{
				state[Nb*i + k - 1] = state[Nb*i + k];
			}
			state[Nb*i + Nb - 1] = tmp;
			s++;
		}
	}
}

void inv_shift_rows(uint8_t *state) 
{
	uint8_t s, tmp;
	for (uint8_t i = 1; i < 4; i++) 
	{
		s = 0;
		while (s < i) 
		{
			tmp = state[Nb*i + Nb - 1];
			
			for (uint8_t k = Nb - 1; k > 0; k--)
			{
				state[Nb*i + k] = state[Nb*i + k - 1];
			}
			state[Nb*i + 0] = tmp;
			s++;
		}
	}
}

void multiplicationforMIX(uint8_t *a, uint8_t *b, uint8_t *c) 
{

	c[0] = gmult(a[0], b[0]) ^ gmult(a[3], b[1]) ^ gmult(a[2], b[2]) ^ gmult(a[1], b[3]);
	c[1] = gmult(a[1], b[0]) ^ gmult(a[0], b[1]) ^ gmult(a[3], b[2]) ^ gmult(a[2], b[3]);
	c[2] = gmult(a[2], b[0]) ^ gmult(a[1], b[1]) ^ gmult(a[0], b[2]) ^ gmult(a[3], b[3]);
	c[3] = gmult(a[3], b[0]) ^ gmult(a[2], b[1]) ^ gmult(a[1], b[2]) ^ gmult(a[0], b[3]);
	//uint8_t a[] = {0x02, 0x01, 0x01, 0x03}; 
}

void mix_columns(uint8_t *state) 
{
	uint8_t a[] = {0x02, 0x01, 0x01, 0x03}; // a(x) = {02} + {01}x + {01}x2 + {03}x3
	uint8_t colomn[4], res[4];
	for (uint8_t j = 0; j < Nb; j++) 
	{
		for (uint8_t i = 0; i < 4; i++)
		{
			colomn[i] = state[Nb*i+j];
		}
		multiplicationforMIX(a, colomn, res);
		for (uint8_t i = 0; i < 4; i++) 
		{
			state[Nb*i+j] = res[i];
		}
	}
}

void inv_mix_columns(uint8_t *state) 
{
	uint8_t a[] = {0x0e, 0x09, 0x0d, 0x0b}; // a(x) = {0e} + {09}x + {0d}x2 + {0b}x3
	uint8_t colomn[4], res[4];
	for (uint8_t j = 0; j < Nb; j++) 
	{
		for (uint8_t i = 0; i < 4; i++) 
		{
			colomn[i] = state[Nb*i + j];
		}
		multiplicationforMIX(a, colomn, res);
		for (uint8_t i = 0; i < 4; i++) 
		{
			state[Nb*i + j] = res[i];
		}
	}
}

void add_round_key(uint8_t *state, uint8_t *expandedkey, uint8_t r) 
{
	for (uint8_t c = 0; c < Nb; c++) 
	{
		state[Nb*0 + c] = state[Nb*0 + c] ^ expandedkey[4*Nb*r + 4*c + 0]; 
		state[Nb*1 + c] = state[Nb*1 + c] ^ expandedkey[4*Nb*r + 4*c + 1];
		state[Nb*2 + c] = state[Nb*2 + c] ^ expandedkey[4*Nb*r + 4*c + 2];
		state[Nb*3 + c] = state[Nb*3 + c] ^ expandedkey[4*Nb*r + 4*c + 3];	
	}
}


void sub_word(uint8_t *key) 
{
	for (uint8_t i = 0; i < 4; i++) 
	{
		key[i] = s_box[key[i]];
	}
}

void rot_word(uint8_t *key) 
{
	uint8_t tmp;
	tmp = key[0];
	for (uint8_t i = 0; i < 3; i++) 
	{
		key[i] = key[i+1];
	}
	key[3] = tmp;
}

void addition(uint8_t *a, uint8_t *b, uint8_t *c) 
{
	c[0] = a[0]^b[0];
	c[1] = a[1]^b[1];
	c[2] = a[2]^b[2];
	c[3] = a[3]^b[3];
}

uint8_t R[] = {0x00, 0x00, 0x00, 0x00};
 
uint8_t * Rcon(uint8_t i) 
{	
	if (i == 1) 
	{
		R[0] = 0x01;
	} 
	else if (i > 1) 
	{
		R[0] = 0x02;
		i--;
		while (i > 1) 
		{
			R[0] = gmult(R[0], 0x02);
			i--;
		}
	}
	return R;
}

void aes_key_expansion(uint8_t *key, uint8_t *expandedkey) 
{
	uint8_t tmp[4];
	uint8_t len = Nb*(Nr+1);
	for (uint8_t i = 0; i < Nk; i++) 
	{
		expandedkey[4*i+0] = key[4*i+0];
		expandedkey[4*i+1] = key[4*i+1];
		expandedkey[4*i+2] = key[4*i+2];
		expandedkey[4*i+3] = key[4*i+3];
	}
	for (uint8_t i = Nk; i < len; i++) 
	{
		tmp[0] = expandedkey[4*(i-1)+0];
		tmp[1] = expandedkey[4*(i-1)+1];
		tmp[2] = expandedkey[4*(i-1)+2];
		tmp[3] = expandedkey[4*(i-1)+3];
		if (i%Nk == 0) 
		{
			rot_word(tmp);
			sub_word(tmp);
			addition(tmp, Rcon(i/Nk), tmp);
		} 
		else if (Nk > 6 && i%Nk == 4) 
		{
			sub_word(tmp);
		}
		expandedkey[4*i+0] = expandedkey[4*(i-Nk)+0]^tmp[0];
		expandedkey[4*i+1] = expandedkey[4*(i-Nk)+1]^tmp[1];
		expandedkey[4*i+2] = expandedkey[4*(i-Nk)+2]^tmp[2];
		expandedkey[4*i+3] = expandedkey[4*(i-Nk)+3]^tmp[3];
	}
}

void aes_cipher(uint8_t *in, uint8_t *out, uint8_t *expandedkey) 
{
	uint8_t state[4*Nb];
	for (uint8_t i = 0; i < 4; i++) 
	{
		for (uint8_t j = 0; j < Nb; j++) 
		{
			state[Nb*i+j] = in[i+4*j];
		}
	}
	add_round_key(state, expandedkey, 0);
	for (uint8_t r = 1; r < Nr; r++) 
	{
		sub_bytes(state);
		shift_rows(state);
		mix_columns(state);
		add_round_key(state, expandedkey, r);
	}
	sub_bytes(state);
	shift_rows(state);
	add_round_key(state, expandedkey, Nr);
	for (uint8_t i = 0; i < 4; i++) 
	{
		for (uint8_t j = 0; j < Nb; j++) 
		{
			out[i+4*j] = state[Nb*i+j];
		}
	}
}

void aes_inv_cipher(uint8_t *in, uint8_t *out, uint8_t *expandedkey) 
{
	uint8_t state[4*Nb];
	for (uint8_t i = 0; i < 4; i++) 
	{
		for (uint8_t j = 0; j < Nb; j++) 
		{
			state[Nb*i+j] = in[i+4*j];
		}
	}
	add_round_key(state, expandedkey, Nr);
	for (uint8_t r = Nr-1; r >= 1; r--) 
	{
		inv_shift_rows(state);
		inv_sub_bytes(state);
		add_round_key(state, expandedkey, r);
		inv_mix_columns(state);
	}
	inv_shift_rows(state);
	inv_sub_bytes(state);
	add_round_key(state, expandedkey, 0);
	for (uint8_t i = 0; i < 4; i++) 
	{
		for (uint8_t j = 0; j < Nb; j++) 
		{
			out[i+4*j] = state[Nb*i+j];
		}
	}
}